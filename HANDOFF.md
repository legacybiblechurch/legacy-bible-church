...
- **Finding a new song:** the Studio triggers the GitHub Action `worship-prep.yml` with the song name → `scripts/prep/run.py --song` searches YouTube and runs the **Exact Lyrics Engine** with whatever it can reach from GitHub's servers (captions, the song's verified library words). If that is not enough to verify every line, the draft is marked `needsAudio` and the **church Mac listener** (`scripts/helper/`, a launchd job that runs every minute) fetches the recording, runs the engine with three independent listens, and pushes the result. The Studio polls and shows "Listening to the recording…" meanwhile.
...
### The church Mac listener (one-time)
